...
static int ax3000_sgpio_set(struct gpio_chip *chip, unsigned int offset, int value)
{
	u32 bank = GET_BANK(offset / 2);
	u32 position = GET_POS(offset / 2);
...
static int ax3000_sgpio_dir_out(struct gpio_chip *chip, unsigned int offset, int value)
{
	if (offset % 2) {
		if (chip->set)
