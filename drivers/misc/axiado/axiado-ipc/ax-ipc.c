...
#include <linux/mm.h>
#include "ax-ipc-gpio.h"
#include "ax-ipc.h"

static LIST_HEAD(device_list);
static DECLARE_BITMAP(minors, AXDEV_MAX_DEVICES);

#ifdef CONFIG_IPC_GPIO
static struct ax_ipc_device *pm0_axdev;
#endif
...
/**
 * @brief Sends an IPC request to the remote CPU with the provided data.
 *
 * This function prepares an IPC packet and transfers it to the remote CPU.
 * The function checks for valid parameters, including the availability of
 * the IPC device and buffer size constraints.
 *
 * This IPC API should only be used to send data to sysmgr.
 * Currently there is no provision to get the data from sysmgr to kernel
 *
 * @param data Pointer to the data to be sent via IPC.
 * @param data_len Length of the data in bytes.
 *
 * @return
 * - 0 on success.
 * - -EINVAL if invalid parameters are provided (e.g., null pointers or
 *   data length exceeding buffer size).
 * - Other error codes as returned by `txfr_to_remote_cpu()`.
 */

#ifdef CONFIG_IPC_GPIO
int ax_ipc_kern_request(void *data, int data_len)
{
	struct ipc_pkt_s pkt;

	if (!pm0_axdev || !pm0_axdev->rx_queue ||
	    data_len > AX_IPC_KERN_BUFF_LEN) {
		pr_err("Invalid parameter\n");
		return -EINVAL;
	}

	unsigned char *phy_mem = (unsigned char *)pm0_axdev->kern_ipc_buf;

	memcpy(phy_mem, data, data_len);
	/** Fill in the ipc packet*/
	pkt.endpoint_id = AX_EP_SYSMGR_A53_PROXY;
	pkt.event = EVT_GPIO_OPS_SIG;

	pkt.data = pm0_axdev->shared_mem_phy_addr;
	pkt.is_pointer = 1;
	pkt.length = data_len;

	pr_debug("%s(%d) :Endpoint= %x\n", __func__, __LINE__, pkt.endpoint_id);
	pr_debug("%s(%d) = %x\n", __func__, __LINE__, pkt.event);
	pr_debug("%s(%d) = %x\n", __func__, __LINE__, pkt.length);
	pr_debug("%s(%d) = %x\n", __func__, __LINE__, pkt.data);
	return txfr_to_remote_cpu(pm0_axdev, &pkt);
}
EXPORT_SYMBOL(ax_ipc_kern_request);

int ax_ipc_gpio_request(struct sysmc_gpio_req_s *gpio_req)
{
	return ax_ipc_kern_request((void *)gpio_req,
				   sizeof(struct sysmc_gpio_req_s));
}
EXPORT_SYMBOL(ax_ipc_gpio_request);
#endif
/**
...
/*
 * Use subsys_initcall() instead of module_init() to ensure IPC driver
 * initializes before GPIO driver. subsys_initcall() runs earlier in
 * the boot sequence than module_init().
 */

#ifndef CONFIG_IPC_GPIO
module_platform_driver(ax_ipc_driver);
#else
static int __init ax_ipc_init(void)
{
	return platform_driver_register(&ax_ipc_driver);
}
subsys_initcall(ax_ipc_init);

static void __exit ax_ipc_exit(void)
{
	platform_driver_unregister(&ax_ipc_driver);
}
module_exit(ax_ipc_exit);
#endif
