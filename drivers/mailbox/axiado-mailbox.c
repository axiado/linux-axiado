								    ...
								    bool *serviced)
{
	 ...
	 * what's here is usable. But do count clearing a real error as
	 * @serviced — otherwise an error that asserts the IRQ with no data
	 * behind it would report IRQ_NONE despite handling a real event,
	 * risking the core's spurious-interrupt detector.
	 */
	...
	*serviced = true;
	...
	/*
	 * readsl(), unlike the TX path: @buf is priv->rx_buffer, allocated
	 * by this driver via devm_kzalloc() and therefore always u32-aligned.
	 * readsl() does no endian conversion, which is fine here since AX3005
	 * and its mailbox peers are little-endian only.
	 */
...
static irqreturn_t axiado_rx_thread(int irq, void *dev_id)
{
	...
	/*
	 * Drain every queued frame before returning: IRQF_ONESHOT masks this
	 * channel while the thread runs, and a frame arriving mid-loop isn't
	 * guaranteed a fresh trigger once we unmask, so anything left behind
	 * here can stall indefinitely. cond_resched() bounds worst-case CPU
	 * time under a sustained peer; a capped/budgeted drain instead would
	 * reintroduce that same stuck-frame risk.
	 */
	do {
