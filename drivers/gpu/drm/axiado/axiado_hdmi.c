...
void axiado_hdmi_write_edid(struct axiado_hdmi *hdmi, struct edid *edid_data)
{
	int i;
	u8 *data;
	size_t edid_length;
				       ...
				       struct drm_display_mode *mode)
{
	...
	rc = drm_hdmi_avi_infoframe_from_display_mode(&frame.avi,
					      &hdmi->connector, mode);
	if (rc < 0) {
		DRM_DEV_ERROR(hdmi->dev, "Failed to setup AVI infoframe: %d\n", rc);
		return rc;
	}
	...
	return axiado_hdmi_upload_frame(hdmi, rc, &frame, INFOFRAME_AVI, 0, 0, 0);
}
...
static int axiado_hdmi_connector_get_modes(struct drm_connector *connector)
{
	...
	if (edid) {
		...
		axiado_hdmi_write_edid(hdmi, edid);
		kfree(edid);
	}
	return ret;
}
