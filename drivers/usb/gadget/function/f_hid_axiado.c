...
/*
 * f_hid_axiado.c -- USB AXIADO HID HPM bridge function driver
 *
...
int hidg_req_complete;
struct ax_dev_node {
	struct f_hidg *ax_hidg;
	struct list_head list;
};

static LIST_HEAD(ax_dev_list);
			    ...
			    size_t count, loff_t *offp)
{
	...
	return ax_hpm_bridge_data_forward(buffer, count, hidg->report_desc);
}

int ax_hpm_bridge_data_forward(const char *buffer, int count, __u8 *report_des)
{
	struct f_hidg *hidg = NULL;
	char *rdesc = (char *)report_des;
	struct ax_dev_node *pos;
	struct usb_request *req;
	unsigned long flags;
	ssize_t status = -ENOMEM;

	list_for_each_entry(pos, &ax_dev_list, list) {
		if (pos->ax_hidg != NULL &&
		    !(memcmp(rdesc, pos->ax_hidg->report_desc,
			     pos->ax_hidg->report_desc_length))) {
			hidg = pos->ax_hidg;
			break;
		}
	}
...
try_again:
	...
	memcpy((char *)req->buf, buffer, count);
	spin_lock_irqsave(&hidg->write_spinlock, flags);
...
EXPORT_SYMBOL_GPL(ax_hpm_bridge_data_forward);
...
static int hidg_set_alt(struct usb_function *f, unsigned int intf,
			unsigned int alt)
{
	struct usb_composite_dev		*cdev = f->config->cdev;
	struct f_hidg				*hidg = func_to_hidg(f);
...
static int hidg_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	list_add(&node->list, &ax_dev_list);
