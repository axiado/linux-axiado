			...
			enum crg_cmd_type type, u32 param0, u32 param1)
{
	...
	tmp = crg_reg_read(&uccr->control);
	if (tmp & CRG_U3DC_CTRL_RUN)
		check_complete = true;

	if (check_complete) {
		tmp = crg_reg_read(&uccr->cmd_control);
	...
	crg_reg_write(&uccr->cmd_param0, param0);
	crg_reg_write(&uccr->cmd_param1, param1);

	/*ignore CMD IOC, in uboot no irq is*/
	tmp = CRG_U3DC_CMD_CTRL_ACTIVE |
		CRG_U3DC_CMD_CTRL_TYPE(type);
	crg_reg_write(&uccr->cmd_control, tmp);
...
void knock_doorbell(struct crg_gadget_dev *crg_udc, int DCI)
{
	...
	crg_reg_write(&uccr->doorbell, tmp);
}
		...
		struct crg_udc_request *udc_req_ptr)
{
	...
	if (usb_endpoint_xfer_control(udc_ep_ptr->desc)) {
		dev_dbg(crg_udc->dev, "crg_udc_queue_trbs control\n");
...
static int set_ep_halt(struct crg_gadget_dev *crg_udc, int DCI)
{
	...
	do {
		tmp = crg_reg_read(&uccr->ep_running);
	} while ((tmp & param0) != 0);
...
static int crg_udc_ep_disable(struct usb_ep *ep)
{
	...
	crg_reg_write(&uccr->ep_enable, 0x1 << udc_ep->DCI);
...
static void enable_setup_event(struct crg_gadget_dev *crg_udc)
{
	...
	dev_dbg(crg_udc->dev, "before setup en config1[0x%p]=0x%x\n",
		&uccr->config1, crg_reg_read(&uccr->config1));

	val = crg_reg_read(&uccr->config1);
	val |= CRG_U3DC_CFG1_SETUP_EVENT_EN;
	crg_reg_write(&uccr->config1, val);
...
static int init_event_ring(struct crg_gadget_dev *crg_udc, int index)
{
	...
	/**************************/
	crg_reg_write(&uicr->erstsz, CRG_ERST_SIZE);
	crg_reg_write(&uicr->erstbalo, lower_32_bits(udc_event->erst.dma));
	crg_reg_write(&uicr->erstbahi, upper_32_bits(udc_event->erst.dma));
	crg_reg_write(&uicr->erdplo,
		lower_32_bits(udc_event->event_ring.dma) | CRG_U3DC_ERDPLO_EHB);
	crg_reg_write(&uicr->erdphi, upper_32_bits(udc_event->event_ring.dma));

	crg_reg_write(&uicr->iman, (CRG_U3DC_IMAN_INT_EN | CRG_U3DC_IMAN_INT_PEND));
	crg_reg_write(&uicr->imod, (0L << 0) | (4000L << 0));
...
static int init_device_context(struct crg_gadget_dev *crg_udc)
{
	...
	crg_reg_write(&uccr->dcbaplo, lower_32_bits(crg_udc->ep_cx.dma));
	crg_reg_write(&uccr->dcbaphi, upper_32_bits(crg_udc->ep_cx.dma));

	dev_dbg(crg_udc->dev, "dcbaplo[0x%p]=0x%x\n", &uccr->dcbaplo, crg_reg_read(&uccr->dcbaplo));
	dev_dbg(crg_udc->dev, "dcbaphi[0x%p]=0x%x\n", &uccr->dcbaphi, crg_reg_read(&uccr->dcbaphi));
...
static int reset_data_struct(struct crg_gadget_dev *crg_udc)
{
	...
	val = crg_reg_read(&uccr->control);
	val &= (~(CRG_U3DC_CTRL_INT_EN | CRG_U3DC_CTRL_RUN));
	crg_reg_write(&uccr->control, val);

	dev_dbg(crg_udc->dev, "capability[0x%p]=0x%x\n", &uccr->capability,
			crg_reg_read(&uccr->capability));
	...
	crg_reg_write(&uccr->config0, tmp);
...
static void crg_udc_start(struct crg_gadget_dev *crg_udc)
{
	...
	dev_dbg(crg_udc->dev, "config0[0x%p]=0x%x\n", &uccr->config0, crg_reg_read(&uccr->config0));
	...
	crg_reg_write(&uccr->control, val);
	dev_dbg(crg_udc->dev, "%s, control=0x%x\n", __func__, crg_reg_read(&uccr->control));
}
...
void crg_udc_clear_portpm(struct crg_gadget_dev *crg_udc)
{
	...
	tmp = crg_reg_read(&uccr->u3portpmsc);
	...
	crg_reg_write(&uccr->u3portpmsc, tmp);
...
void crg_udc_reinit(struct crg_gadget_dev *crg_udc)
{
	...
	dev_dbg(crg_udc->dev, "ep_enable=0x%x\n", crg_reg_read(&uccr->ep_enable));
	dev_dbg(crg_udc->dev, "ep_running=0x%x\n", crg_reg_read(&uccr->ep_running));

	/* disable all the endpoints */
	tmp = crg_reg_read(&uccr->ep_enable);
	crg_reg_write(&uccr->ep_enable, tmp);
	for (i = 0; i < 50; i++) {
		tmp = crg_reg_read(&uccr->ep_enable);
		if (tmp == 0)
			break;
...
static void crg_vbus_detect(struct crg_gadget_dev *crg_udc, int enable)
{
	if (enable) {
		...
		/* Enable the VBUS */
		crg_reg_write(crg_udc->mmio_virt_base + CRG_UDC_VENDOR_REG,
					(crg_reg_read(crg_udc->mmio_virt_base +
						  CRG_UDC_VENDOR_REG) |
					 CRG_UDC_VBUS_DETECT));
...
void set_test_mode_cmpl(struct crg_gadget_dev *crg_udc)
{
	if (crg_udc->set_tm != 0) {
		...
		tmp = crg_reg_read(&uccr->u2portpmsc);
		tmp = SETF_VAR(CRG_U3DC_U2PORTPM_TM, tmp, crg_udc->set_tm);
		crg_reg_write(&uccr->u2portpmsc, tmp);
...
int g_dnl_board_usb_cable_connected(struct crg_gadget_dev *crg_udc)
{
	...
	if (cnt < STATE_USB_LINK_STABLE) {
		tmp = crg_reg_read(crg_udc->mmio_virt_base + CRG_UDC_VENDOR_REG);
		crg_reg_write(crg_udc->mmio_virt_base + CRG_UDC_VENDOR_REG,
			  tmp | 1 << 26);
		...
		crg_reg_write(&uccr->control, tmp);
		tmp = crg_reg_read(&uccr->control);
		dev_dbg(crg_udc->dev, "control = 0x%x\n", tmp);
		mdelay(10);
		pp = (crg_reg_read(crg_udc->mmio_virt_base +
		...
		if (pp) {
			if (crg_udc->device_state < USB_STATE_POWERED) {
				tmp_cfg0 = crg_reg_read(&uccr->config0);
				tmp_cfg0 &= (~0xf0);
				tmp_cfg0 |= 0xf0;
				crg_reg_write(&uccr->config0, tmp_cfg0);
...
int crg_handle_port_status(struct crg_gadget_dev *crg_udc)
{
	...
	/* handle Port Reset */
	portsc_val = crg_reg_read(&uccr->portsc);
	crg_reg_write(&uccr->portsc, portsc_val);
...
int process_event_ring(struct crg_gadget_dev *crg_udc, int index)
{
	...
	crg_reg_write(&uicr->erdphi, upper_32_bits(erdp));
	tmp = lower_32_bits(erdp);
	tmp |= CRG_U3DC_ERDPLO_EHB;
	crg_reg_write(&uicr->erdplo, lower_32_bits(erdp | CRG_U3DC_ERDPLO_EHB));
...
int crg_gadget_handle_interrupt(struct crg_gadget_dev *crg_udc)
{
	...
	tmp_status = crg_reg_read(&uccr->status);

	if (tmp_status & CRG_U3DC_STATUS_SYS_ERR) {
		dev_err(crg_udc->dev, "%s System error happens!!!\n", __func__);
		/*Handle system error*/
		crg_reg_write(&uccr->status, CRG_U3DC_STATUS_SYS_ERR);
	}

	if (tmp_status & CRG_U3DC_STATUS_EINT) {
		int i;

		crg_reg_write(&uccr->status, CRG_U3DC_STATUS_EINT);
...
int crg_gadget_irq_init(struct platform_device *pdev, struct crg_gadget_dev *crg_udc)
{
	int	irq = 0;
	int retval = 0;
...
static int crg_udc_probe(struct platform_device *pdev)
{
	...
	crg_reg_write(crg_udc->mmio_virt_base + CRG_UDC_MODE_REG,
		(crg_reg_read(crg_udc->mmio_virt_base + CRG_UDC_MODE_REG) |
		 CRGUDC_ROLE_DEVICE));
	for (i = 0; i < CRG_RING_NUM; i++) {
...
static int crg_udc_remove(struct platform_device *pdev)
{
	...
	tmp = crg_reg_read(crg_udc->mmio_virt_base + CRG_UDC_MODE_REG) & ~0x1;
	crg_reg_write(crg_udc->mmio_virt_base + CRG_UDC_MODE_REG, tmp);
