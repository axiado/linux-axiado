				  ...
				  u8 *resp, u16 total)
{
	...
	/* Copy header into resp buffer and read the exact remainder */
			      ...
			      struct i2c_msg *msgs, int num)
{
	...
	for (tries = 0; tries < 50; tries++) {
		...
		if (msg_type == RI_MSG_NOT_READY) {
			dev_dbg(h->dev, "resp not ready (seq=%u), retrying\n", hdr_local.seq);
			msleep(RI_SLEEP_MS);
			continue;
		}
		...
		if (msg_type != RI_MSG_XFER_RESULT || hdr_local.seq != seq) {
			dev_dbg(h->dev, "skip resp type=0x%02x seq=%u (want %u)\n",
				msg_type, hdr_local.seq, seq);
		...
		dev_err(h->dev, "xfer err: seq=%u ret=%d\n", seq, ret);
		goto out;
	}

	ret = -ETIMEDOUT;
	dev_err(h->dev, "xfer timeout: seq=%u\n", seq);
...
static int ri_host_probe(struct i2c_client *client)
{
	...
	dev_info(&client->dev, "virtual-mux-host registered as i2c-%d (crc=%d)\n",
