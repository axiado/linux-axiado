...
EXPORT_SYMBOL(axiado_pcie_bar_init);
				...
				enum axiado_pcie_dma_mode mode)
{
	...
	iowrite32(d_cfg.src_inf, ax_pcie->bridge + offset[0]);
	iowrite32(d_cfg.dest_inf, ax_pcie->bridge + offset[1]);
	iowrite32(d_cfg.src_addr_31_0, ax_pcie->bridge + offset[2]);
	iowrite32(d_cfg.src_addr_63_32, ax_pcie->bridge + offset[3]);
	iowrite32(d_cfg.dest_addr_31_0, ax_pcie->bridge + offset[4]);
	iowrite32(d_cfg.dest_addr_63_32, ax_pcie->bridge + offset[5]);
	iowrite32(d_cfg.length, ax_pcie->bridge + offset[6]);
	iowrite32(ctrl, ax_pcie->bridge + offset[7]);
}
				  ...
				  enum axiado_pcie_dma_channel chan_num)
{
	...
	iowrite32(DMA_START, ax_pcie->bridge + offset);
}
			    ...
			    enum axiado_pcie_dma_channel chan_num)
{
	...
	offset = find_dma_control_offset(port, chan_num);
	return ((ioread32(ax_pcie->bridge + offset) & 0x1) != 0x0);
}
...
unsigned int axiado_pcie_ocm_csr_read(struct platform_device *pdev)
{
	...
	return ioread32(ax_pcie->bridge + REG_PCIE_X2_CSR_INT_OCM_OFFSET);
}
					...
					u32 val)
{
	...
	iowrite32(val, (ax_pcie->bridge + REG_PCIE_X2_CSR_INT_OCM_OFFSET));
}
